...
2. Manage notes:
   ...
   # Filter by tag
   crunes run notes list --tag database

   # Search note content
   crunes run notes list --search "vectors"

   # Add a new note with native array tags
   crunes run notes add --title "AI Study" --content "Research embedding models" --tags ai,research

   # Soft-delete a note
   crunes run notes delete --id 1

   # View all notes including soft-deleted ones
   crunes run notes list --all
   ```
