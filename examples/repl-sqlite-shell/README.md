...
An interactive SQLite query shell demonstrating the `runRepl` and `argsRepl` lifecycles, alongside a one-shot `run` lifecycle on the same rune.
...
- **`{ type: 'done', message }` signal** — returned by `runRepl` to end the session cleanly on `exit`, `quit`, or `\q`
- **Both lifecycles on one rune** — `crunes run sqlite-shell` for one-shot queries, `crunes run-repl sqlite-shell` for an interactive shell
- **Dynamic prompt** — returns `` `[${rowCount} rows]> ` `` after SELECT queries so the caller always sees how many rows came back
- **`runRepl` permissions** — the `config.json` `runRepl` permission block is separate from `run`; both must be declared for a rune that uses both lifecycles
...
# Interactive shell
crunes run-repl sqlite-shell

# Machine-readable REPL (AI agent / pipe-friendly)
printf "SELECT * FROM books\nexit\n" | crunes run-repl --format jsonl sqlite-shell
```
...
- `crunes run-repl sqlite-shell` — opens an interactive prompt; type any SQL to execute it; type `exit`, `quit`, or `\q` to end the session
...
### `runRepl` permissions in `config.json`

The `run` and `runRepl` lifecycles use separate permission namespaces. A rune that uses both must declare both:

```json
"permissions": {
  "run":     { "allow": ["sqlite.read:./state::books", "sqlite.write:./state::books"] },
  "runRepl": { "allow": ["sqlite.read:./state::books", "sqlite.write:./state::books"] }
}
```
